...
- **It is never scheduled.** An open base contributes nothing to a tick, so it declares no `every`, no `lease` and no `validUntil`, and nothing is stored, so it takes no `codec`. All four are rejected at construction whenever the property is present - an explicit `undefined` included - as are a list beside `anyParams` and an `anyParams` that is not `true`.
