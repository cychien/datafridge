...
Capacity is a separate question from rate, and it now has a separate answer: `createFridge({ maxPerTick })` bounds how many entries one tick loads and takes on - the invocation, not upstream - and defaults to 500. What does not fit is left exactly as it was and comes back in `RunReport.deferred`, most overdue first.
