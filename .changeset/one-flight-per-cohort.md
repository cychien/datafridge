...
Holder ids are the caller's, and `random` is injectable, so uniqueness is never assumed: a store refuses a `holder` that already holds a live permit rather than letting it take a second or overwrite the first, and the caller mints another and takes the slot. The compatibility suite pins that, because the two obvious wrong answers - overwrite, or raise - breach the ceiling and lose the call respectively.
...
Two costs the first cut left behind are gone. A tick no longer looks up a row per registry entry the page did not reach: a short page *is* the whole table, so those names are known to have no row without asking, and past a page the lookups go out in bounded batches. And `sweepFlights` keeps a running flight until its expiry is well behind, because deleting one lets the next caller restart at generation one - the very value a late leader would still be holding, and the thing the generation guard rests on.
