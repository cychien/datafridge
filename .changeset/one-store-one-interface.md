...
`createPoller`, `cronPoller`, and `PollerDO` each take one `store`. `createReader({ store })` needs nothing but `readResult` to answer, so a read-only consumer can supply just that; a store that also offers `readSchedule` lets a miss tell a fetch that is about to land from a retry already scheduled for later.
