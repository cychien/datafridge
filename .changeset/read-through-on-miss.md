...
`swrRefresh` now fires only for a stale result. A miss is served by the read itself, so a second refresh would have been redundant.

`read` takes params positionally on both sides: `read(name, params?, options?)`. `poller.read`'s `params` option is gone.
